...
from tests.helper.user_subscription_helpler import user_subscription_create_invalid_subscription_json, \
    ...
    user_subscription_response_json, user_subscription_update_json
...
def test_subscriptions_create_ok(test_app, user_extra_data_db, user_subscription_db, mocker):
    mocker.patch.object(user, 'get_by_user_id', return_value=user_extra_data_db)
...
def test_subscriptions_get_no_filter_results(test_app, user_extra_data_db, user_subscription_db, mocker):
    mocker.patch.object(user, 'get_by_user_id', return_value=user_extra_data_db)
    mocker.patch.object(user_subscription, 'get_subscriptions_by_user_id', return_value=[user_subscription_db])
    response = test_app.get("/api/v1/1/subscriptions")
    assert response.status_code == 200
...
def test_subscriptions_get_inactive_filter_results(test_app, user_extra_data_db, user_subscription_db, mocker):
    mocker.patch.object(user, 'get_by_user_id', return_value=user_extra_data_db)
    mocker.patch.object(user_subscription, 'get_subscriptions_by_user_id', return_value=[user_subscription_db])
...
def test_subscriptions_get_active_filter_results(test_app, user_extra_data_db, user_subscription_db, mocker):
    mocker.patch.object(user, 'get_by_user_id', return_value=user_extra_data_db)
    mocker.patch.object(user_subscription, 'get_subscriptions_by_user_id', return_value=[user_subscription_db])
...
def test_subscriptions_update_ok(test_app, user_extra_data_db, user_subscription_db, mocker):
    mocker.patch.object(user, 'get_by_user_id', return_value=user_extra_data_db)
    mocker.patch.object(subscription, 'get_by_subscription_plan', return_value=gold_subscription_db)
    mocker.patch.object(user_subscription, 'get_subscription_by_user_and_subscription',
                        return_value=user_subscription_db)
    mocker.patch.object(user_subscription, 'update', return_value=user_subscription_db)
