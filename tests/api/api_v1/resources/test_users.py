...
complete_user_info_db = json.loads(
    '{ \
      "id": 1, \
      "first_name": "name", \
      "last_name": "lastname", \
...
other_user_complete_user_info_db = json.loads(
    '{ \
      "id": 1, \
      "first_name": "name", \
      "last_name": "lastname", \
...
complete_user_info_expected = json.loads(
    '{ \
      "first_name": "name", \
      "last_name": "lastname", \
      "email": "some@mail.com.ar", \
...
user_complete = UserAccount(**complete_user_info_db)
other_complete = UserAccount(**other_user_complete_user_info_db)
...
def test_user_ok_basic_info(test_app, mocker):
    ...
    assert response.json() == basic_user_info
...
def test_user_ok_complete_info(test_app, mocker):
    ...
    assert response.status_code == 200
    assert response.json() == complete_user_info_expected
...
def test_users_ok_users_ok(test_app, mocker):
    mocker.patch.object(user, 'get_multi', return_value=[user_complete, user_complete])
    response = test_app.get("/api/v1/users")
    assert response.status_code == 200
    assert response.json() == {'results': [basic_user_info, basic_user_info]}


def test_users_ok_users_ok_filter(test_app, mocker):
    ...
    assert response.status_code == 200
    assert response.json() == {'results': [basic_user_info]}
...
def test_users_create_ok(test_app, mocker):
    ...
    response = test_app.post("/api/v1/users/", data=json.dumps(basic_user_info))
    assert response.status_code == 200
    assert response.json() == basic_user_info
